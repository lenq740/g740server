...
/**
 * @file
 * G740Server, автоматический генератор объектов доступа к данным DataSource по описанию структуры базы данных.
 *
 ...
 */
	function __construct($params=Array()) {
		if ($params['echo']) $this->isEcho=true;
...
SQL;
			...
			while($recParams=$this->pdoFetch($qParams)) {
				if ($dsF['type']=='list' || $dsF['type']=='icons' || $dsF['type']=='radio') {
					if ($recParams['name']=='list') {
						$dsF['list']=str_replace("\n",';',trim($recParams['val']));
					}
...
/**
 * @file
 * Автосгенеренный источник данных для таблицы '{$tableName}'
 */
 ...
 */
	public function getDataSource($params=Array()) {
		...
		$resultGetFields=strTabShift($this->_getDataSourceFields($fields, $tableName),2);
		...
		$resultGetRef=strTabShift($this->_getDataSourceTableRef($params),1);
		...
		$result=<<<PHP
/** Класс DataSource - автоматически построенная основа источника данных для таблицы '{$tableName}'
 *
 * Этот класс обеспечивает базовый функционал работы источника данных для таблицы '{$tableName}'.
 * Строится автоматически по описанию из модели данных. 
 * Для расширения функционала источника данных необходимо породить потомка от этого класса.
 */
 ...
 *
 * @return	Array описание связей источника данных
 */
 ...
 *
 * @param	Array	{$D}params параметры запроса
 * @return	string список полей для запроса select
 */
 ...
 *
 * @param	Array	{$D}params параметры запроса
 * @return	string секция from для запроса select
 */
...
/** Этот метод возвращает секцию where для запроса select
 *
 * @param	Array	{$D}params параметры запроса
 * @return	string секция where для запроса select
 */
 ...
 *
 * @param	Array	{$D}params параметры запроса
 * @return	string строка order by для запроса select
 */
 ...
 *
 * @param	Array	{$D}params параметры запроса
 * @return	string XML описание полей
 */
...
function getDataSourceModify($name) {
	global $_registerDataSourceModify;
	...
	if ($_registerDataSourceModify[$name]) return $_registerDataSourceModify[$name];

	$fileNameModify=pathConcat(
		getCfg('path.root'),
		getCfg('path.root.datasources', pathConcat(getCfg('path.root.php'),'datasources')),
		'modify',
		"{$name}-modify.php"
	...
	if (file_exists($fileNameModify)) {
		...
		if ($obj instanceof DataSourceModify) $_registerDataSourceModify[$name]=$obj;
	}
	
	$result=$_registerDataSourceModify[$name];
	if (!$result) $result=null;
	return $result;
}
/// Кэш модификаторов DataSourceModify
$_registerDataSourceModify=Array();
